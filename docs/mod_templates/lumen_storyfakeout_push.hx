/* FNF Lumen Engine - Story Mode Fakeout Example [PUSH]
 * by bobbyDX
...
function onChangeWeek(weekFileName) {
    // Check if the selected week is Weekend 1 and if it has been completed
    if (weekFileName == "wknd1" 
        && StoryMenuState.weekCompleted.exists("wknd1") 
        && StoryMenuState.weekCompleted.get("wknd1")) {
        
        // Check if the high score for wknd1 is above 0, just as a check if the complete check fails
        ...
        if (highScore > 0) {
            ...
            weekData.songs = [
                ...
                ["Lit Up", 1],
                ["2hot", 1],
                ["Blazin", 1] // the secret song is here, doesn't need to be put in week JSON
            ];

            //trace("Custom tracklist for " + weekFileName + " applied!");
        } else {
            //trace("High score for " + weekFileName + " is 0. Default tracklist loaded.");
        }
    } else {

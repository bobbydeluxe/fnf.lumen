...
// code by bobbydx
// this is a basic helper class to construct custom freeplay backing cards
// why the hell do i need this? becuse scripting backing cards becomes impossible without it
// idk why but the lumen codebase is like that

class BackingCardHScript {
  ...
  public function addObject(name:String) {
    #if HSCRIPT_ALLOWED
    if (hscriptObjects.exists(name)) return;
    var obj = hscriptObjects.get(name);
    group.add(obj);
    hscriptObjects.set(name, obj);
    if (callback != null)
      callback("onObjectAdded", [name]);
    #end
  }

...
class FreeplayMetaJSON {
    ...
    public var allowErectVariants = true;// this does NOTHING, its just for p-slice 3.0+ compatibility shit
...
class FreeplayMeta {
    public static function getMeta(songId:String):FreeplayMetaJSON {
        var meta_file = Paths.getTextFromFile('data/songs/${Paths.formatToSongPath(songId)}/metadata.json');
        if(meta_file != null){
